...
def main():
    ...
    carteira   = pd.read_csv(GOLD / "carteira_banco_mes.csv",       encoding="utf-8")

    produtos   = pd.read_csv(GOLD / "produtos_mais_vendidos.csv",   encoding="utf-8")

    localidade = pd.read_csv(GOLD / "inadimplencia_localidade.csv", encoding="utf-8")

    share      = pd.read_csv(GOLD / "share_mercado.csv",            encoding="utf-8")

    qualidade  = pd.read_csv(GOLD / "qualidade_dados.csv",          encoding="utf-8")

    oport_vuln = pd.read_csv(GOLD / "ataque_vulneravel.csv",        encoding="utf-8")

    oport_rec  = pd.read_csv(GOLD / "ataque_recuperar.csv",         encoding="utf-8")
    ...
    ov_label = j((ov.location_name + " \u00d7 " + ov.product_name).tolist())
    ...
    or_label = j((or_.location_name + " \u00d7 " + or_.product_name).tolist())
...
<aside>
  ...
  <nav>
    ...
    <a class="active" onclick="go(0)" id="nav0"><span class="nav-icon">&#x1F4CA;</span> Visão Geral</a>

    <a onclick="go(1)" id="nav1"><span class="nav-icon">&#x1F3AF;</span> Share de Mercado</a>

    <a onclick="go(2)" id="nav2"><span class="nav-icon">&#x26A0;&#xFE0F;</span> Risco & Inadimplência</a>
  ...
  <div class="sidebar-footer">
    ...
    <div>Dados: Jan–Dez 2025</div>

    <div class="threat-level">Nível de Ameaça<br><span>&#9632; HIDRA ATIVA</span></div>
...
<main>
  ...
  <div class="topbar">
    ...
    <div>
      ...
      <h1>Central de Inteligência Financeira</h1>

      <p>Missão: conter a expansão da Hidra nos mercados financeiros</p>
  ...
  <div class="page active" id="p0">
    ...
    <div class="insight">
      ...
      <span class="tag">BRIEFING</span> O Banco Shield opera <strong>{ks['sc']:,} contratos</strong> com carteira de <strong>R${ks['ss']}M</strong>, superando a Hidra em volume. Inadimplência sob controle em <strong>{ks['si']}%</strong> vs {ks['hi']}% da Hidra. Qualidade dos dados: <strong>{ks['sq']}%</strong> de registros válidos.
    ...
    <div class="g g3" style="grid-template-columns:repeat(3,1fr)">
      ...
      <div class="card {'red' if ks['si'] > ks['hi'] else ''}">
        ...
        <div class="ct">Inadimplência 30+ DPD</div>
    ...
    <div class="sl">Evolução da Carteira — 2025</div>

    <div class="g g2">

      <div class="chart">

        <div class="chart-title">Saldo Total por Mês (R$ M)</div>

        <div class="chart-sub">Exposição financeira acumulada — Shield vs Hidra</div>
      ...
      <div class="chart">
        ...
        <div class="chart-title">Novos Contratos por Mês</div>

        <div class="chart-sub">Volume de originação mensal comparado</div>
  ...
  <div class="page" id="p1">
    ...
    <div class="insight">
      ...
      <span class="tag">ALERTA</span> A Hidra domina o segmento de <strong>Seguros ({seg_hidra_pct}%)</strong>. Shield lidera em Financiamento, Empréstimo e Conta. Oportunidade: atacar o nicho de Seguros onde a Hidra avança.

    </div>

    <div class="sl">Disputa por Território</div>
    ...
    <div class="g g2">
      ...
      <div class="chart">
        ...
        <div class="chart-sub">Participação relativa — quem domina cada segmento</div>
      ...
      <div class="chart">
        ...
        <div class="chart-sub">Total de contratos — onde está o maior campo de batalha</div>
  ...
  <div class="page" id="p2">
    ...
    <div class="insight">
      ...
      <span class="tag">AMEAÇA</span> Consórcio Xandar (Hidra) lidera inadimplência com <strong>2.19%</strong>. São Paulo e Wakanda são as localidades de maior risco para o Shield. Risk score médio da Hidra (~0.14) é 55% maior que o Shield (~0.09).
    ...
    <div class="g g2">
      ...
      <div class="chart">
        ...
        <div class="chart-title">Top Produtos — Índice de Inadimplência (%)</div>

        <div class="chart-sub">Azul = Shield · Vermelho = Hidra</div>
      ...
      <div class="chart">
        ...
        <div class="chart-title">Top Localidades — Índice de Inadimplência (%)</div>

        <div class="chart-sub">Concentração geográfica de risco</div>
  ...
  <div class="page" id="p3">
    ...
    <div class="insight">
      ...
      <span class="tag">GOVERNANÇA</span> <strong>{grafias_corrigidas} registros</strong> com nome de banco inválido foram corrigidos automaticamente. Shield: <strong>{int(shield_row.err_id_duplicado)} IDs duplicados</strong>, {int(shield_row.err_periodo_invalido)} períodos fora do range. Hidra: <strong>{int(hidra_row.err_periodo_invalido)} períodos inválidos</strong>, {int(hidra_row.err_produto_fk)} FKs de produto quebradas.
    ...
    <div class="g g4">
      ...
      <div class="card">
        ...
        <div class="ct">Período Inválido</div>
    ...
    <div class="insight" style="border-color:#f5a62344;background:linear-gradient(135deg,#1a150a,#2a1f0a);margin-bottom:18px">
      ...
      <span class="tag" style="background:#3a2a10;color:{GOLD_C}">PONTO DE ATENÇÃO</span>

      <strong style="color:{GOLD_C}">{bgf_total} contratos</strong> apresentam saldo em aberto maior que o valor financiado no mês —

      <strong style="color:{SC}">{bgf_shield} no Shield</strong> (diferença média R$ {bgf_media_s:,.2f}) e

      <strong style="color:{HC}">{bgf_hidra} na Hidra</strong> (diferença média R$ {bgf_media_h:,.2f}).

      Desses contratos, <strong style="color:{GOLD_C}">{bgf_adim_s}% dos casos Shield</strong> e <strong style="color:{GOLD_C}">{bgf_adim_h}% dos casos Hidra</strong> são adimplentes (dpd = 0),

      o que indica que a diferença provavelmente não é decorrente de juros por inadimplência.

      Os registros foram mantidos na base. Recomenda-se validação com a área de negócio.
    ...
    <div class="g g2">
      ...
      <div class="chart">
        ...
        <div class="chart-sub">Quantidade de registros com violação por categoria</div>
      ...
      <div class="chart">
        ...
        <div class="chart-title">Relatório de Integridade por Banco</div>

        <div class="chart-sub">Visão consolidada das anomalias detectadas no pipeline</div>

        <table>

          <thead><tr><th>Banco</th><th>Total</th><th>ID Dup</th><th>Período</th><th>Prod FK</th><th>Local FK</th><th>Valor Neg</th><th>Status</th></tr></thead>
  ...
  <div class="page" id="p4">
    ...
    <div class="insight">
      ...
      <span class="tag">MISSÃO</span> Duas frentes de ataque identificadas: nichos onde a <strong>Hidra está vulnerável</strong> (risco alto, carteira deteriorando) e nichos onde a <strong>Hidra domina e o Shield está perdendo terreno</strong> — prioridade estratégica de recuperação.

    </div>

    <div class="sl">Frente 1 — Hidra Vulnerável (entrar agora com baixo risco)</div>
    ...
    <div class="g g2">
      ...
      <div class="chart">
        ...
        <div class="chart-sub">Risco alto + inadimplência alta = janela de entrada para o Shield</div>
      ...
      <div class="chart">
        ...
        <div class="chart-sub">Quanto maior, mais a carteira da Hidra está deteriorada</div>
    ...
    <div class="sl">Frente 2 — Recuperar Território (Shield perdendo para a Hidra)</div>

    <div class="g g2">

      <div class="chart">

        <div class="chart-title">Score de Recuperação de Mercado</div>

        <div class="chart-sub">Hidra forte + Shield fraco = território a reconquistar</div>
      ...
      <div class="chart">
        ...
        <div class="chart-sub">Distância a recuperar em cada nicho estratégico</div>
...
function buildPage(i) {{
  ...
  if(i===2) {{
    ...
    new Chart(document.getElementById('rp').getContext('2d'), hbar({rp_n}, [
      ...
      {{label:'Inadimplência %',data:{rp_v},backgroundColor:{rp_col},borderRadius:4}}

    ], true));

    new Chart(document.getElementById('rl').getContext('2d'), hbar({rl_n}, [

      {{label:'Inadimplência %',data:{rl_v},backgroundColor:{rl_col},borderRadius:4}}
